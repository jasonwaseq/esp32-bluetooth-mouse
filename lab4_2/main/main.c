...
#define HIDD_DEVICE_NAME            "HID-ESP32C3-Mouse!"
...
void hid_demo_task(void *pvParameters)
{
    ...
    int8_t x_step = 20;       // horizontal movement per iteration
    uint8_t buttons = 0;      // no buttons pressed

    while (1) {
        if (sec_conn) {
            // Move mouse right
            esp_hidd_send_mouse_value(hid_conn_id, buttons, x_step, 0);

            // Stop movement
            esp_hidd_send_mouse_value(hid_conn_id, buttons, 0, 0);

            // Pause for 5 seconds
            vTaskDelay(5000 / portTICK_PERIOD_MS);

        } else {
            vTaskDelay(500 / portTICK_PERIOD_MS); // check connection periodically
        }
    }
}
